...
if (greenChicks[0].rewardIngredients[0].id !== GAME_INGREDIENTS.leaf.id
  || greenChicks.slice(1).some((route) => route.rewardIngredients[1].id !== GAME_INGREDIENTS.leaf.id)
  || JSON.stringify(INGREDIENT_SLOT_WEIGHTS) !== JSON.stringify({ primary: 0.5, secondary: 0.3, special: 0.2 })) {
  throw new Error("Green chick leaf slots do not match the first-visit/40-visit plan");
}
...
try {
  ...
  if (routes.length !== 3
    ...
    || routes[1].rewardItems[1].name !== "나뭇잎"
    || routes[2].rewardItems[1].name !== "나뭇잎") {
  ...
  console.log("GREEN_CHICK_LEAF_DROPS_OK avocado=primary-from-1 cabbage/cactus=secondary-from-40 overallDrop=15%");
} finally {
  await browser.close();
}
