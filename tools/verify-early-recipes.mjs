...
if (CORE_PROGRESSION.length !== 45 || RECIPE_PROGRESSION.length !== 59 || EARLY_RECIPE_CATALOG.length !== 14) {
...
if (earlyIngredientIds.size !== 9 || JSON.stringify(earlyRecipes.map((route) => route.recipeName)) !== JSON.stringify(expectedEarlyNames)) {
  throw new Error(`Expected nine pre-300-visit recipes from nine ingredients: ${JSON.stringify({ ingredients: [...earlyIngredientIds], recipes: earlyRecipes.map((route) => route.recipeName) })}`);
}
...
try {
  ...
  if (await page.locator(".recipe-catalog-card").count() !== 59) throw new Error("The UI recipe catalog did not expand to 59 cards");
  ...
  if (current.recipes.catalogTotal !== 59
