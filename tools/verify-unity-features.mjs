...
const dismissTutorial = async () => {
  for (let index = 0; index < 6 && await page.locator("#chef-dialogue").isVisible(); index += 1) {
...
try {
  ...
  if (navLabels.join(",") !== "테마,레시피,손님,노하우") throw new Error(`Unexpected bottom navigation: ${navLabels.join(",")}`);
  ...
  await page.screenshot({ path: path.join(out, "01-four-button-layout.png"), fullPage: true });
  ...
  for (const name of ["조명", "테이블", "조리기구"]) {
    ...
    if (!(await page.locator("#install-panel").isVisible())) throw new Error(`Install panel did not open: ${name}`);
  ...
  await page.evaluate(() => {
    ...
    const cuttingBoard = window.CHICK_TABLE_SOURCE.InstallFacility.find((row) => row.areaType === 1 && row.facilityType === 8);
    ...
    saved.crafting = { ingredients: { 30003: 1, 30039: 1 }, history: [], selected: [] };
  ...
  await page.locator('[data-action="select-ingredient"][data-id="30003"]').click();
  ...
  if (crafted.recipes.owned !== 2 || crafted.recipes.levels[2] !== 1
    || Number(crafted.progression.ingredients[30003] || 0) !== 0) throw new Error(`Manual recipe discovery failed: ${JSON.stringify(crafted.recipes)}`);
  ...
  if (saved.ownedRecipes[2]?.level !== 1 || saved.metrics.recipesCrafted !== 1) throw new Error("Crafted recipe state mismatch");
